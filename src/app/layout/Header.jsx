...
import { HeaderSkeleton } from "@/src/components/LoadingSkeleton";
...
function Header() {
  ...
  const { loading, currentBoard, setEditBoard } = useContext(BoardTaskContext);
                ...
                <ul className="absolute top-[40px] right-[5px] bg-magnumGrey p-3 shadow-2xl shadow-slate-800 rounded-lg z-20">
                  <li>
                    <a
                      ...
                      onClick={async () => {
                        let columnsData = await fetchABoardsDetails(
                          currentBoard
                        );
